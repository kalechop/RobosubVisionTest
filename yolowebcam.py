...
while True:
    success, img = "counterclockwise.png"    #cap.read()
    ...
    for r in results:
        ...
        for box in boxes:
            ...
            if classNames[cls] == "Counterclockwise Banner"  and conf>.5:
                cv2.rectangle(img,(x1,y1),(x2,y2),(0,0,255),3)
                cvzone.putTextRect(img, f'{classNames[cls]} {conf}',(max(0,x1),max(35,y1)), scale = 1, thickness=2)#defult is 3 for both
                print("Counterclockwise Banner: ", find_angle_to_object(x1,y1,x2,y2))
                c_x = x1 + int((x2-x1)/2)
                c_y = y1 + int((y2-y1)/2)
                cv2.circle(img, (c_x,c_y), radius=10, color=(0, 255, 0), thickness=10)
            if classNames[cls] == "Clockwise Banner" and conf > .5:
                cv2.rectangle(img,(x1,y1),(x2,y2),(255,0,0),3)
                cvzone.putTextRect(img, f'{classNames[cls]} {conf}',(max(0,x1),max(35,y1)), scale = 1, thickness=2)#defult is 3 for both
                print("Clockwise Banner: ", find_angle_to_object(x1,y1,x2,y2))  
                c_x = x1 + int((x2-x1)/2)
                c_y = y1 + int((y2-y1)/2)
                cv2.circle(img, (c_x,c_y), radius=10, color=(0, 255, 0), thickness=10)
